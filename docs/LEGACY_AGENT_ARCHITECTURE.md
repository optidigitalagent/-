# Legacy Agent Architecture — preserved baseline

This document preserves the previous root `AGENTS.md` instructions so the existing Gmail/Telegram job-notification implementation is not lost while the repository is upgraded into the Antonov Digital Freelance Revenue Engine.

Before relying on a path below, verify it against the current `optidigital-agent/` tree. New project-brain rules in root `AGENTS.md` take precedence where instructions conflict.
